...
Private headlineStyleName As String
Private mainTitleStyleName As String
Private subTitleStyleName As String
...
Public Sub ToPixivTextFormat()
    ...
    Dim styleSettingFilePath As String: styleSettingFilePath = ActiveDocument.AttachedTemplate.Path & "\pixiv_style_name.txt"
    If Not readSettingToPixivTextFormat(styleSettingFilePath) Then MsgBox "Failed to load style settings file.", vbExclamation, "Please confirm.": Exit Sub
...
Private Function readSettingToPixivTextFormat(style_setting_file_path As String) As Boolean
    readSettingToPixivTextFormat = False
    
    Dim styleText As String: styleText = readUtf16PixivBinaryFile(style_setting_file_path)

    If Len(styleText) = 0 Then Exit Function
    
    headlineStyleName = getRegMatchsForPixivText(styleText, "(^|\n)headlineStyleName:(.+?)($|\n)")(0).SubMatches(1)
    If Len(headlineStyleName) = 0 Then Exit Function
    
    mainTitleStyleName = getRegMatchsForPixivText(styleText, "(^|\n)mainTitleStyleName:(.+?)($|\n)")(0).SubMatches(1)
    If Len(mainTitleStyleName) = 0 Then Exit Function
    
    subTitleStyleName = getRegMatchsForPixivText(styleText, "(^|\n)subTitleStyleName:(.+?)($|\n)")(0).SubMatches(1)
    If Len(subTitleStyleName) = 0 Then Exit Function

    readSettingToPixivTextFormat = True
End Function
...
Private Function convParagraphToPixivTextFormat(paragraph_object As Paragraph)
    ...
    Then
        If _
            ...
            And _
            (InStr(1, paragraph_object.Style, headlineStyleName) = 1) _
        Then
            convParagraphToPixivTextFormat = Array("[chapter:", "]")
        ElseIf _
            (Not ((existsStartSerif) And (existsStopSerif))) _
            And _
            ((paragraph_object.Style <> mainTitleStyleName) And (paragraph_object.Style <> subTitleStyleName) And (InStr(1, paragraph_object.Style, headlineStyleName) = 0)) _
        Then
